 ...
 */
void squared_error(int B, const float* y, int incy, const float* Z,
    int ldZ, float* l, int incl);
 ...
 */
void squared_error_grad(int B, const float* y, int incy,
    const float* m, int ldm, float* d, int incd);

/**
 * Take one step of an Adam optimizer.
 */
void adam(const int P, const int t, const float gamma,
    const float beta1, const float beta2, const float epsilon, float* m, float* v,
